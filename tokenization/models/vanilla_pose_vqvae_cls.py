...
from .classifier import MaskClassifier as CLASSIFIER

smpl_type='smplh'
import os
current_dir = os.path.dirname(os.path.realpath(__file__))
body_model_path = os.path.join(current_dir, '..', '..', 'data/body_models', smpl_type)
body_model = SMPLHLayer(model_path=f'../data/body_models/{smpl_type}', num_betas=10, ext='pkl')
...
def amputation_mapping(mapped_joints, body_pose):
    batch_size, num_joints, _, _ = body_pose.shape
    body_pose = body_pose.clone()
    joint_mask = torch.zeros(batch_size, num_joints, 1, 1, device=body_pose.device)
    max_joints = max(len(j) for j in mapped_joints) if mapped_joints else 0
    if max_joints == 0:
        return body_pose

    mapped_joints_padded = torch.full((batch_size, max_joints), -1, device=body_pose.device)
    for i, joints in enumerate(mapped_joints):
        if joints:  # Only update if the list is non-empty
            mapped_joints_padded[i, :len(joints)] = torch.tensor(joints, device=body_pose.device)

    # Create a mask of valid indices (indices in [0, num_joints-1])
    valid_mask = (mapped_joints_padded >= 0) & (mapped_joints_padded < num_joints)
    # Replace invalid indices with 0 (dummy index) to avoid out-of-bound errors.
    safe_indices = mapped_joints_padded.clone()
    safe_indices[~valid_mask] = 0

    # Scatter only where indices are valid
    joint_mask.scatter_(1, safe_indices.unsqueeze(-1).unsqueeze(-1),
                         valid_mask.float().unsqueeze(-1).unsqueeze(-1))
    body_pose = body_pose * (1 - joint_mask)
    return body_pose
...
class PoseSPDecoderV1(nn.Module):
    ...
    def forward(self, x, global_step=None):
        ...
        if self.rot_type == 'rot6d':
            ...
            # print(pred_pose_rotmat[0])
            pred_mask = self.classifier(pred_pose_rotmat.reshape(pred_pose.shape[0], 21, 9))
        ...
        if self.mesh_inference:
            ...
            elif global_step > 700:
                mapped_joints = [JOINT_MAP[label.item()] if label.item() in JOINT_MAP else [] for label in torch.argmax(pred_mask, dim=-1)]
                pred_pose_rotmat = amputation_mapping(mapped_joints, pred_pose_rotmat)
...
class VanillaTokenizer(nn.Module):
    ...
    def forward(self, x, global_step=None):
        ...
        # Encode
        x_encoder = self.encoder(x, global_step=None)
        ## quantization
        x_quantized, loss, perplexity  = self.quantizer(x_encoder)
        ## decoder
        x_decoder, pred_mask = self.decoder(x_quantized, global_step)
...
class DecodeTokens(nn.Module):
    ...
    def forward(self, logits, global_step=None):
        decode_feat = self.quantizer.dequantize_logits(logits)
        pose_out, pred_mask = self.decoder(decode_feat.permute(0,2,1), global_step=global_step)
        pred_mask = torch.argmax(pred_mask, dim=-1)
        return pose_out['pred_pose_body_6d'], pred_mask
