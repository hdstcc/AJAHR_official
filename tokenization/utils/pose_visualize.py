...
from .rotation_conversions import euler_angles_to_matrix, matrix_to_axis_angle
...
def get_body_model(model_type, smpl_type='smplx'):
    from smplx import SMPLH, SMPLX, SMPLHLayer
    smpl_model = SMPLHLayer(model_path=f'../data/body_models/{smpl_type}', num_betas=10, ext='pkl')
    return smpl_model
...
def visualize_body(vertices, faces, mesh_viewer, num_vertices=None, index=0, show_fig=False, side_view=False):
    ...
    body_mesh = trimesh.Trimesh(vertices=c2c(vertices[index]), faces=faces, vertex_colors=np.tile(colors['grey'], (num_vertices, 1)))
    if side_view:
...
def visualize_from_mesh(smpl_type, input_batch, output_batch, iter_str, save_dir=''):
    ...
    pose_aa = input_batch['gt_pose_body']
    ...
    side_euler = torch.FloatTensor([[0,-1.4,0]])
    side_rot = euler_angles_to_matrix(side_euler, 'XYZ')
    
    for pose_idx in range(num_renders):
        gt_pose = pose_aa[[pose_idx]]
    ...
    if side_view:
        for pose_idx in range(num_renders):
            ...
            body = body_model(body_pose=gt_pose, global_orient=side_rot)
            vertices = body.vertices.detach()
            faces = body_model.faces
            imgs.append(visualize_body(vertices, faces, mesh_viewer))
...
#         body = body_model(body_pose=gt_pose.view(gt_pose.shape[0], -1))
#         vertices = body.vertices.detach()
#         faces = body_model.faces
#         imgs.append(visualize_body(vertices, faces, mesh_viewer))
...
#         for pose_idx in range(num_renders):
#             gt_pose = gt_pose_body[[pose_idx]]
#             body = body_model(body_pose=gt_pose.view(gt_pose.shape[0], -1), global_orient=torch.FloatTensor([[0,-1.4,0]]).cuda())
#             vertices = body.vertices.detach()
#             faces = body_model.faces
#             imgs.append(visualize_body(vertices, faces, mesh_viewer))
