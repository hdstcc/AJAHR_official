...
from .rotation_conversions import matrix_to_rotation_6d
...
def reset_err_list(type='tr'):
    err_list = {f'{type}/curr_pose_recons': 0.,
                ...
                f'{type}/curr_mask_loss': 0.,
                f'{type}/curr_accuracy': 0.,}
    if type == 'tr':
        err_list.update({
            f'{type}/curr_loss': 0.
        })
...
def init_best_scores():
    best_scores = {
        ...
        f'val/best_mask_err': 1e8,
        f'val/best_accuracy': 1e8
    }
    return best_scores
...
def forward_mask(gt_mask, pred_mask):
    gt_mask, pred_mask = gt_mask.squeeze(), pred_mask.squeeze()
    pred_mask_classes = torch.argmax(pred_mask, dim=-1)  # (batch, 21)
    loss = F.cross_entropy(pred_mask, gt_mask, reduction='mean')
    accuracy = accuracy_score(gt_mask.cpu().detach().numpy(), pred_mask_classes.cpu().detach().numpy()) * 100
    return loss, accuracy

def save_results_func(batch, output, results):
    valid_joints = [*range(1,22)]
    pose_aa = batch['gt_pose_body']
    gt_pose_body = matrix_to_rotation_6d(pose_aa.view(-1, 3, 3)).float().to('cuda')
    save_aa_gt = gt_pose_body.view(-1, 21 * 3).cpu().detach().numpy()
...
def eval_pose_vqvae(hparams, val_loader, net, logger, writer, nb_iter, out_dir, val_disp_iter, best_scores):
    ...
    for batch_idx, batch in enumerate(tqdm.tqdm(val_loader)):
        ...
        # gt_label = batch['labels'].cuda().long()
        dataset_name = batch['dataset_name'][0]

        with torch.no_grad():
            output, loss_commit, perplexity = net(gt_pose)

        gt_pose_6d = matrix_to_rotation_6d(gt_pose)
        ####

        pose_error = calculate_pose_reconstruction_error(gt_pose_6d, output['pred_pose_body_6d'])
        ...
        # mask_error, accuracy = forward_mask(gt_label, output['pred_mask'])
        ...
        # err_list['val/curr_mask_loss'] += mask_error.item()
        # err_list['val/curr_accuracy'] += accuracy
