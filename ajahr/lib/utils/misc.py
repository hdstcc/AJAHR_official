...
def load_pretrained(cfg, backbone, smpl_head, classifier, strict=False, ckpt=None, is_train_state=True):
    if is_train_state == False:
        if cfg.get('ckpt_path', None):
            ...
            if os.path.exists(PRETRAINED_WEIGHTS_path):
                ...
                pt_model_cls = torch.load(PRETRAINED_WEIGHTS_path, map_location='cpu')['state_dict']
                prepare_statedict(backbone, pt_model, 'backbone',strict=False)
                prepare_statedict(smpl_head, pt_model, 'smpl_head',strict=False)
                prepare_statedict(classifier, pt_model_cls, 'classifier', strict=False)
            else:
                log.warning(f'Missing full pretrained model from {PRETRAINED_WEIGHTS_path}')
                exit(1)
        return backbone, smpl_head, classifier
