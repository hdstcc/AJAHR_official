...
class Renderer:
    ...
    def expand_uv_coords(self, vertices, faces, uv_coords, uv_indices):
        """
        UV 좌표를 정점의 개수에 맞게 확장합니다.
        
        Args:
            vertices (np.array): 정점 배열 (6890, 3).
            faces (np.array): 면 배열 (13776, 3).
            uv_coords (np.array): UV 좌표 배열 (7576, 2).
            uv_indices (np.array): UV 좌표 인덱스 배열 (13776, 3).
        
        Returns:
            expanded_uv_coords (np.array): 확장된 UV 좌표 배열 (6890, 2).
        """
        expanded_uv_coords = np.zeros((vertices.shape[0], 2), dtype=np.float32)
        
        for face, uv_face in zip(faces, uv_indices):
            for v_idx, uv_idx in zip(face, uv_face):
                expanded_uv_coords[v_idx] = uv_coords[uv_idx]
        
        return expanded_uv_coords

    def merge_textures(self, texture1, texture2):
        """
        두 텍스처를 병합합니다. texture2의 알파 채널을 사용하여 texture1 위에 덮어씁니다.
        """
        # texture2를 texture1과 동일한 크기로 리사이즈
        texture2_resized = cv2.resize(texture2, (texture1.shape[1], texture1.shape[0]))

        # texture2_resized의 알파 채널을 기반으로 마스크 생성
        mask = texture2_resized[:, :, 3] > 0  # texture2의 알파 채널이 0보다 큰 부분만 선택

        # texture1 위에 texture2_resized 덮어쓰기
        merged_texture = texture1.copy()
        merged_texture[mask] = texture2_resized[mask]  # texture2_resized로 덮어쓰기
    ...
    def render_rgba_multiple(
        ...
        for vvv, ttt in zip(vertices, cam_t):
            mesh = self.vertices_to_trimesh(vvv, ttt.copy(), mesh_base_color, rot_axis, rot_angle)
            mesh_list.append(pyrender.Mesh.from_trimesh(mesh))
