    ...
    if cfg.MODEL.BACKBONE.TYPE == 'vit':
        ...
        if cfg.MODEL.BACKBONE.get('PRETRAINED_WEIGHTS', None) and load_weights:
            ...
            try:
                ...
                print('--------------------------------------------------------')
                print(f'Loading backbone weights partially from {PRETRAINED_WEIGHTS_path}')
                print('--------------------------------------------------------')

                backbone = prepare_statedict(backbone, pt_model, 'backbone', strict=False)
        return backbone
    else:
