...
class SMPLTokenDecoderHead(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.decpose = TokenClassfier(dim, tokenizer_checkpoint_path_amp = cfg.MODEL.TOKENIZER_CHECKPOINT_PATH_AMP, \
                                    tokenizer_checkpoint_path_normal = cfg.MODEL.TOKENIZER_CHECKPOINT_PATH_NORMAL, \
    ...
    def forward(self, x, y=None, mask=None, **kwargs):
        ...
        for i in range(self.cfg.MODEL.SMPL_HEAD.get('IEF_ITERS', 1)):
            ...
            #init pose shape : [batch, 144]
            #init betas shape : [batch, 10]
            #init cam shape : [batch,]
            ...
            token_out = self.transformer(token, context=x) #[16 (BATCH), 1024]
            token_out = token_out.squeeze(1) # (B, C) 1024
            # Readout from token_out
            pred_grot = self.decpose_grot(token_out)
            pred_bpose, cls_logits_softmax = self.decpose(token_out, mask) 
            pred_handpose = self.decpose_hands(token_out)
            
            # pred_grot, pred_bpose, pred_handpose = self.linear_head.pose_forward(pred_grot, pred_bpose, pred_handpose)
            ...
            # print(pred_grot.shape, pred_bpose.shape, pred_handpose.shape)
            # print(pred_body_pose.shape)
            pred_betas = self.decshape(token_out) + pred_betas
            pred_cam = self.deccam(token_out) + pred_cam

            # pred_betas, pred_cam = self.linear_head.shape_cam_forward(pred_betas, pred_cam)
        ...
        # cls_out = self.linear_head(pred_smpl_params['body_pose'][:, :21, :, :])
